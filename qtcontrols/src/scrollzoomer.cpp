...
ScrollZoomer::ScrollZoomer(QWidget *parent):
...
void ScrollZoomer::rescale()
{
    ...
    else
    {
        if ( !d_inZoom )
        {
            ...
            QwtPlotLayout *layout = plot()->plotLayout();
            d_alignCanvasToScales = layout->alignCanvasToScale(QwtPlot::xBottom);
...
void ScrollZoomer::updateScrollBars()
{
	...
	qDebug() << "moving slider left %f right %f\n" <<  QDateTime::fromTime_t(zoomRect().left()) << QDateTime::fromTime_t(zoomRect().right());
