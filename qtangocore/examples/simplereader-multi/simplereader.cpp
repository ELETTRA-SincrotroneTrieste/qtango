...
int main(int argc, char *argv[])
{
    ...
    {
        source = qApp->arguments().at(i);
        QObject *parentO = new QObject(&a);
        SimpleReaderMulti *reader = new SimpleReaderMulti(parentO);
...
SimpleReaderMulti::SimpleReaderMulti(QObject *parent) : QObject(parent),
...
void SimpleReaderMulti::refresh(const TVariant &v)
{
    double readVal;
    if(v.canConvertToDouble() || v.canConvertToState())
    {
        readVal = v.toDouble();
        formatResult(v);
    }
...
/* optional */
void SimpleReaderMulti::init(const TangoConfigurationParameters *cp)
{
...
void SimpleReaderMulti::formatResult(const TVariant &v)
{
    AttributeInfo ai;
    double d;
