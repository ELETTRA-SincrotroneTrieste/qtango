...
TAppInfo TApplicationsPoolProxyPrivate::getInfo(const QString& serviceName)
{
    ...
    if(!m_dbusConnectionInterface->isValid())
        m_errorMessage = m_dbusConnectionInterface->lastError().message();
    else if(serviceName.contains("TApplications"))
    {
...
bool TApplicationsPoolProxyPrivate::updateTApplicationList()
{
    ...
    {
//        pinfo("TApplicationsPoolProxyPrivate: getting registered service names\n");
        ...
        if(services.isValid())
        {
            ...
            foreach(QString service, services.value())
            {
                if(service.contains("TApplications"))
                {
                    ...
                    if(ai.isEmpty())
                    {
                        qDebug() << "not found in previous list by service " << service;
                        ai = getInfo(service);                       
                    }
                    if(!ai.isEmpty())
                        newList.add(ai);
